...
<template>
  <div class="flex h-full flex-col bg-background">
      ...
      <div class="mx-auto flex items-center bg-card/70 backdrop-blur-[40px] border border-border/60 shadow-[0_8px_30px_rgba(0,0,0,0.04),inset_0_1px_0_rgba(255,255,255,0.1)] rounded-[1.25rem] p-1.5 ring-1 ring-black/5 dark:ring-white/10 transition-[box-shadow,border-color,ring-color] duration-500 hover:shadow-[0_8px_40px_rgba(0,0,0,0.08)] focus-within:shadow-[0_12px_50px_rgba(var(--primary),0.12)] focus-within:border-primary/30 focus-within:ring-primary/20 max-w-[1200px]">
        ...
        <Popover v-model:open="openPopover">
          <PopoverTrigger as-child>
            <Button
              ...
              class="h-11 rounded-[14px] bg-muted/40 hover:bg-muted/80 px-4 font-medium flex items-center gap-2.5 shrink-0 border border-transparent hover:border-border/60 transition-[background-color,border-color] text-[13.5px] shadow-[inset_0_1px_1px_rgba(255,255,255,0.2)] dark:shadow-none"
            >
              ...
              <span v-if="selectedIds.size === 0" class="text-muted-foreground tracking-wide">{{ t('multiExec.selectTarget') }}</span>
              <span v-else class="text-foreground font-bold tracking-tight">{{ t('multiExec.terminalsReady', { count: selectedIds.size }) }}</span>
          ...
          <PopoverContent class="w-[320px] p-0 shadow-[0_20px_60px_-15px_rgba(0,0,0,0.3)] border-border/50 rounded-[18px] overflow-hidden bg-background/95 backdrop-blur-2xl translate-y-3 flex flex-col" align="start">
            ...
            <div class="flex items-center px-4 border-b border-border/40 h-12 shrink-0">
              ...
              <input v-model="searchTarget" :placeholder="t('multiExec.searchServers')" class="flex-1 min-w-0 bg-transparent border-none outline-none focus:ring-0 text-[12.5px] placeholder:text-muted-foreground/50 text-foreground font-medium" />
            </div>
            
            <div v-if="filteredSshConnections.length === 0" class="py-10 text-center text-[12.5px] text-muted-foreground">{{ t('multiExec.noMatch') }}</div>
            ...
            <div v-else class="max-h-[300px] overflow-y-auto p-1.5 custom-scrollbar flex flex-col gap-0.5">
              <div
                ...
                :key="conn.record.id"
                class="group flex items-center gap-3.5 rounded-[12px] px-3.5 py-3 cursor-pointer text-[13px] transition-colors duration-300 hover:bg-primary/10 select-none"
                :class="selectedIds.has(conn.record.id) ? 'bg-primary/5 text-primary' : ''"
                @click="toggleConnection(conn.record.id)"
              >
                <!-- 精致的复选框 -->
                <div class="h-4 w-4 shrink-0 rounded-[6px] border flex items-center justify-center transition-[background-color,border-color,color,box-shadow] duration-300 shadow-sm"
                ...
                <div class="flex items-center gap-3 min-w-0 flex-1 pl-0.5">
                  <div class="h-8 w-8 rounded-[10px] bg-background border border-border/50 flex items-center justify-center shadow-sm shrink-0 transition-[border-color,background-color,scale] duration-300 group-hover:border-primary/30 group-hover:bg-primary/5 group-hover:scale-105" :class="selectedIds.has(conn.record.id) ? 'border-primary/40 bg-primary/10' : ''">
            ...
            <div class="p-2 border-t border-border/40 bg-card/40 backdrop-blur-xl flex gap-2 items-center justify-between shrink-0">
              <span class="text-[11px] font-medium text-muted-foreground pl-2 flex items-center gap-1.5 opacity-80">
                ...
                {{ t('multiExec.selected') }} {{ selectedIds.size }} / {{ sshConnections.length }}
              </span>
              <div class="flex items-center gap-1 pr-1">
                <Button variant="ghost" size="sm" class="h-7 px-3 text-[11.5px] font-semibold rounded-[8px] hover:bg-background hover:shadow-sm transition-[background-color,box-shadow]" @click="selectAll" :disabled="selectedIds.size === sshConnections.length" :class="selectedIds.size === sshConnections.length ? 'opacity-40' : 'text-foreground'">
                  {{ t('multiExec.selectAll') }}
                </Button>
                <Button variant="ghost" size="sm" class="h-7 px-3 text-[11.5px] font-medium rounded-[8px] hover:bg-background hover:text-destructive hover:shadow-sm transition-[background-color,color,box-shadow]" @click="deselectAll" :disabled="selectedIds.size === 0" :class="selectedIds.size === 0 ? 'opacity-40' : 'text-muted-foreground'">
        ...
        <div class="flex-1 relative flex items-center group">
          <input 
            ...
            class="w-full h-11 bg-transparent border-none outline-none focus:ring-0 px-2 text-[14.5px] font-mono text-foreground placeholder:text-muted-foreground/40 transition-colors font-medium"
          ...
          <div class="absolute right-2 opacity-0 group-hover:opacity-100 focus-within:opacity-100 transition-opacity flex items-center">
            <TooltipProvider>
              <Tooltip>
                ...
                <TooltipContent side="bottom"><p>{{ t('multiExec.clear') }}</p></TooltipContent>
              </Tooltip>
            </TooltipProvider>
          </div>
        </div>
              ...
              <div class="flex items-center gap-2.5 px-3 py-2 bg-muted/30 border-b border-border/50 cursor-default select-none group/term transition-colors hover:bg-muted/50">
                <div class="relative flex h-2 w-2 items-center justify-center shrink-0">
                  <div class="absolute inline-flex h-full w-full rounded-full opacity-30"
                       :class="{
                         'bg-df-success animate-ping': sessionStatuses.get(conn.record.id) === 'connected',
                         'bg-df-warning animate-pulse': sessionStatuses.get(conn.record.id) === 'connecting',
                  ...
                  <div
                    ...
                    :class="{
                      'bg-df-success shadow-[0_0_6px_var(--df-success)]': sessionStatuses.get(conn.record.id) === 'connected',
                      'bg-df-warning shadow-[0_0_6px_var(--df-warning)]': sessionStatuses.get(conn.record.id) === 'connecting',
    ...
    <div v-else class="flex-1 overflow-y-auto p-4 sm:p-6 bg-muted/10 flex items-center justify-center relative select-none">
      <div class="flex flex-col items-center justify-center text-center animate-in fade-in zoom-in-[0.98] duration-700 max-w-md mx-auto p-12 rounded-[2.5rem] bg-transparent relative z-10 transition-opacity">
        ...
        <div class="relative h-32 w-32 mb-10 flex items-center justify-center group">
          ...
          <!-- 立体玻璃材质底座 -->
          <div class="h-24 w-24 rounded-[1.75rem] bg-gradient-to-tr from-card to-background border border-border/60 shadow-[0_16px_50px_rgba(0,0,0,0.06),inset_0_1px_0_rgba(255,255,255,0.4)] flex items-center justify-center ring-1 ring-black/5 dark:ring-white/10 relative overflow-hidden transition-transform duration-700 group-hover:scale-110">
            <!-- 表面光泽映射 -->
            ...
            <!-- 主荧幕 Icon -->
            <Monitor class="h-10 w-10 text-muted-foreground/60 drop-shadow-[0_1px_1px_rgba(255,255,255,0.8)] dark:drop-shadow-[0_1px_2px_rgba(0,0,0,0.8)] transition-colors duration-500 group-hover:text-primary/80" />
          </div>
        </div>

        <!-- 极简排版文本 -->
        <div class="space-y-3">
          <h3 class="text-2xl font-bold tracking-tight text-foreground/85">{{ t('multiExec.selectTarget') }}</h3>
          <p class="text-[14.5px] text-muted-foreground/70 max-w-[280px] leading-relaxed mx-auto font-medium">
            {{ sshConnections.length > 0 ? t('multiExec.selectDesc') : t('multiExec.noSshDesc') }}
          </p>
        </div>
      </div>
    </div>
