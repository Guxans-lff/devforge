...
<template>
  <div class="flex h-full flex-col border-l border-border/40 bg-zinc-950/50">
    ...
    <div class="flex items-center gap-2 px-3 py-1.5 border-b border-border/20 shrink-0">
      ...
      <span class="text-[10px] font-bold text-muted-foreground/50 uppercase tracking-wider">{{ t('redis.clients.title') }}</span>
      <span class="text-[9px] font-mono text-primary/50">{{ clients.length }}</span>
      <div class="flex-1" />
      <Button variant="ghost" size="sm" class="h-6 w-6 p-0" @click="loadClients" :disabled="loading">
    ...
    <div class="flex-1 overflow-auto">
      <div v-if="clients.length > 0" class="divide-y divide-border/10">
        <div
          ...
          :key="client.id"
          class="flex items-center gap-2 px-3 py-2 hover:bg-muted/10 text-[11px]"
        >
          ...
          <div class="flex-1 min-w-0">
            ...
            <div class="flex items-center gap-2 mt-0.5">
              <span :class="['text-[9px] px-1 py-0.5 rounded', flagColor(client.flags)]">{{ client.flags }}</span>
              <span class="text-[9px] text-muted-foreground/40">DB{{ client.db }}</span>
              <span v-if="client.cmd" class="text-[9px] font-mono text-primary/50">{{ client.cmd }}</span>
            </div>
          </div>
          <div class="text-right shrink-0 text-[9px] text-muted-foreground/40 space-y-0.5">
            <div>{{ t('redis.clients.age') }}: {{ formatDuration(client.age) }}</div>
            <div>{{ t('redis.clients.idle') }}: {{ formatDuration(client.idle) }}</div>
          </div>
          <button
            class="h-6 w-6 flex items-center justify-center rounded text-muted-foreground/20 hover:text-destructive hover:bg-destructive/10 shrink-0"
      ...
      <div v-else-if="!loading" class="text-muted-foreground/20 text-center py-8 text-[11px]">
