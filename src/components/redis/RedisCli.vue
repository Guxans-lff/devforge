...
<template>
  <div class="flex h-full flex-col border-l border-border/40 bg-background/50">
    ...
    <div class="flex items-center gap-2 px-3 py-1.5 border-b border-border/20 shrink-0">
      ...
      <span class="text-xs font-bold text-muted-foreground/50 uppercase tracking-wider">CLI</span>
    </div>

    <!-- 输出区域 -->
    <div ref="outputRef" class="flex-1 overflow-auto p-3 space-y-3 font-mono text-xs">
      <div v-for="(item, i) in history" :key="i" class="space-y-0.5">
        ...
        >{{ item.result }}</div>
        <div class="text-[10px] text-muted-foreground/25">{{ item.durationMs }}ms</div>
      </div>
    ...
    <div class="flex items-center gap-2 px-3 py-2 border-t border-border/20 shrink-0">
      <span class="text-primary/50 text-xs font-mono font-bold">></span>
      <Input
        v-model="command"
        placeholder="GET key"
