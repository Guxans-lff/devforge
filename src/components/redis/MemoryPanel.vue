...
<template>
  <div class="flex h-full flex-col border-l border-border/40 bg-zinc-950/50">
    ...
    <div class="flex items-center gap-2 px-3 py-1.5 border-b border-border/20 shrink-0">
      ...
      <span class="text-[10px] font-bold text-muted-foreground/50 uppercase tracking-wider">{{ t('redis.memory.title') }}</span>
      <div class="flex-1" />
      <Button variant="ghost" size="sm" class="h-6 w-6 p-0" @click="loadStats" :disabled="loadingStats">
        <RefreshCw class="h-3 w-3" :class="loadingStats && 'animate-spin'" />
      </Button>
    </div>
    ...
    <div class="flex-1 overflow-auto p-3 space-y-4">
      ...
      <div v-if="stats" class="grid grid-cols-2 gap-2">
        <div class="rounded-lg border border-border/20 bg-muted/10 p-2.5">
          <div class="text-[9px] text-muted-foreground/50 uppercase">{{ t('redis.memory.usedMemory') }}</div>
          <div class="text-sm font-bold text-foreground mt-0.5">{{ stats.usedMemoryHuman }}</div>
        </div>
        <div class="rounded-lg border border-border/20 bg-muted/10 p-2.5">
          <div class="text-[9px] text-muted-foreground/50 uppercase">{{ t('redis.memory.peakMemory') }}</div>
          <div class="text-sm font-bold text-foreground mt-0.5">{{ stats.usedMemoryPeakHuman }}</div>
        </div>
        <div class="rounded-lg border border-border/20 bg-muted/10 p-2.5">
          <div class="text-[9px] text-muted-foreground/50 uppercase">{{ t('redis.memory.fragmentation') }}</div>
        ...
        <div class="rounded-lg border border-border/20 bg-muted/10 p-2.5">
          <div class="text-[9px] text-muted-foreground/50 uppercase">{{ t('redis.memory.evicted') }}</div>
      ...
      <!-- MEMORY DOCTOR -->
      <div>
        <Button variant="outline" size="sm" class="h-7 text-[10px] w-full" @click="loadDoctor" :disabled="loadingDoctor">
          <Loader2 v-if="loadingDoctor" class="h-3 w-3 mr-1 animate-spin" />
          {{ t('redis.memory.doctor') }}
        </Button>
        <div v-if="showDoctor" class="mt-2 p-2.5 rounded-lg border border-border/20 bg-muted/10 text-[11px] text-foreground/80 font-mono whitespace-pre-wrap break-words">
          {{ doctorAdvice }}
        </div>
      </div>
      ...
      <div class="space-y-2">
        <div class="flex items-center gap-2">
          <span class="text-[10px] font-bold text-muted-foreground/50 uppercase">{{ t('redis.memory.topKeys') }}</span>
          <div class="flex-1" />
          <Input
            type="number"
            v-model.number="topCount"
            class="h-6 w-16 text-[10px] text-center"
            min="1"
            max="100"
          />
          <Button variant="outline" size="sm" class="h-6 text-[10px] px-2" @click="loadTopKeys" :disabled="loadingTopKeys">
            <Loader2 v-if="loadingTopKeys" class="h-3 w-3 mr-1 animate-spin" />
            {{ t('redis.memory.calculate') }}
          </Button>
        </div>
        <div class="flex items-center gap-1.5 text-[9px] text-amber-500/60 px-1">
        ...
        <div v-if="topKeys.length > 0" class="space-y-0.5">
          <div
            ...
            :key="item.key"
            class="flex items-center gap-2 px-2 py-1.5 rounded hover:bg-muted/10 text-[11px]"
          >
            ...
            <span class="text-[9px] px-1.5 py-0.5 rounded bg-primary/10 text-primary/70 shrink-0">{{ item.keyType }}</span>
