...
import { Eye, EyeOff, FolderOpen, Globe, Hash, User, Lock, Key, Shield, Share2, ShieldCheck } from 'lucide-vue-next'
...
<template>
  <div class="space-y-10">
      ...
      <div class="grid grid-cols-12 gap-4">
        ...
        <div class="col-span-3 space-y-2">
          <div class="flex items-center justify-between px-1">
            ...
            <span class="text-[8px] font-mono text-muted-foreground/50 font-black tracking-tighter uppercase bg-muted/30 px-1 rounded-sm">SSH</span>
          </div>
          <div class="relative group">
      ...
      <div class="space-y-6">
        ...
        <TransitionGroup name="form-fade">
          ...
          <template v-if="localValue.authMethod === 'key'">
            ...
            <div key="passphrase" class="space-y-2 animate-in fade-in zoom-in-95 duration-300 pointer-events-auto">
    ...
    <!-- Section 3: Targeted Resources (ProxyJump) -->
    <section class="space-y-5">
      <div class="flex items-center gap-2">
        <div class="h-1.5 w-1.5 rounded-full bg-primary/40"></div>
      ...
      <Transition name="form-fade">
        <div v-if="localValue.proxyJumpEnabled" class="space-y-2 animate-in fade-in slide-in-from-top-2 duration-300">
          <Label class="text-[11px] uppercase font-bold tracking-normal text-muted-foreground/70 px-1">{{ t('connection.proxyJumpHost') }}</Label>
          <Select
            :model-value="localValue.proxyJumpConnectionId"
            @update:model-value="updateField('proxyJumpConnectionId', $event as string)"
          >
            <SelectTrigger class="h-10 bg-muted/10 border-border rounded-lg transition-[border-color,box-shadow] focus:ring-primary/5 text-xs">
              <template #default>
                <div class="flex items-center gap-2 font-bold">
                  <Share2 class="h-4 w-4 text-primary/70" />
                  <SelectValue :placeholder="t('connection.selectProxyJump')" />
                </div>
              </template>
            </SelectTrigger>
            <SelectContent class="bg-popover border-border rounded-xl shadow-2xl">
              ...
              >
                <div class="flex items-center gap-2">
                  <span class="font-bold">{{ conn.record.name }}</span>
                  <span class="text-[10px] font-mono opacity-40">({{ conn.record.host }})</span>
                </div>
              </SelectItem>
            </SelectContent>
          </Select>
