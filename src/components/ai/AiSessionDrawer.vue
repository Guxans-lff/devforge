  ...
  <Sheet :open="open" @update:open="emit('update:open', $event)">
    <SheetContent side="left" class="w-[320px] p-0">
      <SheetHeader class="px-4 pt-4 pb-3 border-b border-border/50">
        <SheetTitle class="text-sm">{{ t('ai.sessions.title') }}</SheetTitle>
        <SheetDescription class="text-xs text-muted-foreground">
          {{ t('ai.sessions.description') }}
        </SheetDescription>
      </SheetHeader>
          ...
          <div class="px-3 pb-3 space-y-0.5">
            ...
            >
              ...
              <p class="text-xs text-muted-foreground">
                {{ searchQuery ? t('ai.sessions.emptySearch') : t('ai.sessions.emptyHistory') }}
              </p>
            </div>
