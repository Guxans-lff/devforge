...
const gatewayPolicyIssues = computed<Array<{ key: string; tone: Tone; message: string }>>(() => {
  ...
  const activeProvider = providers.find(provider => provider.id === activeProviderId)
    ...
    ?? providers[0]
  return auditGatewayPolicy({
    policy,
    providers,
    primaryProviderId: activeProvider?.id,
    primaryProvider: activeProvider,
    gatewayErrorCount: gatewaySnapshot.value.summary.errorCount,
    singleProviderLevel: 'warning',
    primaryFallbackMessage: 'Fallback 列表包含当前主 Provider，建议只保留真正备用 Provider，主 Provider 降级模型会自动处理。',
  }).map(issue => ({
    key: issue.key,
    tone: issue.level === 'danger' ? 'danger' : 'warn',
