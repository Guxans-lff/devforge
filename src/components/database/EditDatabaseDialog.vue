...
const collationMap: Record<string, { label: string, value: string }[]> = {
  default: [{ label: '默认 (不更改)', value: 'default' }],
  utf8mb4: [
    { label: '默认 (不更改)', value: 'default' },
...
watch(() => props.open, async (val) => {
  if (val) {
    ...
    // Fetch current db charset and collation
    try {
      const result = await dbApi.dbExecuteQuery(
        props.connectionId, 
        `SELECT DEFAULT_CHARACTER_SET_NAME, DEFAULT_COLLATION_NAME FROM information_schema.SCHEMATA WHERE SCHEMA_NAME = '${props.database}';`
      )
      
      if (result && result.rows && result.rows.length > 0) {
        const row = result.rows[0] as unknown[]
        const currentCharsetStr = typeof row[0] === 'string' ? row[0] : ''
        const currentCollationStr = typeof row[1] === 'string' ? row[1] : ''
        
        const currentCharset = currentCharsetStr.toLowerCase()
        const currentCollation = currentCollationStr.toLowerCase()
        
        // Find if they exist in our predefined lists
        const charsetExists = charsetOptions.some(opt => opt.value === currentCharset)
        if (charsetExists) {
          charset.value = currentCharset
          // Temporarily set isLoading to true to prevent the watch(charset) from resetting collation
          await nextTick()
          
          const collationList = collationMap[currentCharset] || []
          const collationExists = collationList.some(opt => opt.value === currentCollation)
          if (collationExists) {
            collation.value = currentCollation
          }
        }
      }
    } catch (e) {
...
async function handleSave() {
  if (charset.value === 'default' && collation.value === 'default') {
  ...
  try {
    let sql = `ALTER DATABASE \`${props.database}\``
  ...
  <Dialog :open="open" @update:open="(val) => !isSaving && emit('update:open', val)">
    <DialogContent class="sm:max-w-[480px] p-0 overflow-hidden bg-background/95 backdrop-blur-xl border-border/40 shadow-2xl">
      ...
      <div class="px-6 py-6 space-y-6">
        <!-- 数据库名称展示（不可编辑） -->
        <div class="space-y-2">
          <Label class="text-sm font-medium text-foreground/80 flex items-center justify-between">
            数据库名称
            <span class="text-xs font-normal text-muted-foreground">不可修改</span>
          </Label>
          <Input
            :model-value="database"
            class="w-full bg-muted/60 text-foreground font-semibold border-border/40 cursor-not-allowed h-10 opacity-70"
            disabled
          />
        </div>
      ...
      <div class="px-6 py-4 border-t border-border/20 bg-muted/10 flex justify-end gap-3">
        <Button variant="outline" :disabled="isSaving" @click="emit('update:open', false)" class="shadow-sm">
          取消
        </Button>
        <Button variant="default" :disabled="isSaving" @click="handleSave" class="min-w-[90px] shadow-sm">
