...
devforge/
├── src/                        # 前端 Vue 源码
│   ├── api/                    # Tauri invoke 封装（26 个模块）
│   ├── components/             # 通用组件（~180 个）
│   │   ├── database/           # 数据库相关（查询面板、表编辑器、ER 图、性能监控等）
...
│   ├── composables/            # Vue composables（42 个）
...
│   │   └── WelcomeView.vue     # 欢迎页
│   ├── types/                  # TypeScript 类型定义（39 个）
│   └── locales/                # i18n 语言包（中/英）
├── src-tauri/
│   └── src/
│       ├── commands/           # Tauri 命令（28 个模块）
...
| 模块 | 状态 | 说明 |
|------|------|------|
| 数据库管理 | ✅ 完成 | MySQL/PostgreSQL/SQLite 查询、表编辑、备份恢复、导入导出、ER 图、性能分析、索引建议、架构对比 |
| SSH 终端 | ✅ 完成 | 远程命令执行、终端录制回放、命令片段 |
| SFTP 文件管理 | ✅ 完成 | 文件上传下载、书签、权限管理、文件 diff |
| 批量执行 | ✅ 完成 | 多主机并发命令执行 |
