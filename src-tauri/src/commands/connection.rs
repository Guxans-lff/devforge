use chrono::Utc;
use tauri::{command, Manager};
use uuid::Uuid;

use crate::models::connection::{
    Connection, ConnectionGroup, CreateConnectionRequest, PoolConfig, SslConfig, UpdateConnectionRequest,
};
use crate::services::credential::CredentialManager;
use crate::services::db_engine::DbEngine;
use crate::services::storage::Storage;
use crate::utils::error::AppError;

use std::sync::Arc;

/// Storage 内部使用 SqlitePool（本身线程安全且支持并发），无需外层 Mutex
pub type StorageState = Arc<Storage>;

// --- Connection CRUD ---

#[command]
pub async fn create_connection(
    app: tauri::AppHandle,
    req: CreateConnectionRequest,
) -> Result<Connection, AppError> {
    let storage = app.state::<StorageState>().inner().clone();
    let now = Utc::now().timestamp_millis();
    let id = Uuid::new_v4().to_string();

    let password = req.password;
    let conn = Connection {
        id: id.clone(),
        name: req.name,
        connection_type: req.connection_type,
        group_id: req.group_id,
        host: req.host,
        port: req.port,
        username: req.username,
        config_json: req.config_json,
        color: req.color,
        sort_order: 0,
        created_at: now,
        updated_at: now,
    };

    storage.create_connection(&conn).await?;

    if let Some(password) = password {
        if !password.is_empty() {
            CredentialManager::save(&id, &password)?;
            // Verify credential was stored correctly
            let stored = CredentialManager::get(&id)?;
            if stored.as_deref() != Some(password.as_str()) {
                log::warn!("Credential verification failed for new connection {}: stored_len={:?}, expected_len={}",
                    id, stored.as_ref().map(|s| s.len()), password.len());
            } else {
                log::info!("Credential saved and verified for connection {} (len={})", id, password.len());
            }
        }
    }

    Ok(conn)
}

#[command]
pub async fn update_connection(
    app: tauri::AppHandle,
    id: String,
    req: UpdateConnectionRequest,
) -> Result<Connection, AppError> {
    let storage = app.state::<StorageState>().inner().clone();
    let mut conn = storage.get_connection(&id).await?;

    if let Some(name) = req.name {
        conn.name = name;
    }
    if let Some(group_id) = req.group_id {
        conn.group_id = Some(group_id);
    }
    if let Some(host) = req.host {
        conn.host = host;
    }
    if let Some(port) = req.port {
        conn.port = port;
    }
    if let Some(username) = req.username {
        conn.username = username;
    }
    if let Some(config_json) = req.config_json {
        conn.config_json = config_json;
    }
    if let Some(color) = req.color {
        conn.color = Some(color);
    }

    conn.updated_at = Utc::now().timestamp_millis();
    storage.update_connection(&conn).await?;

    if let Some(password) = req.password {
        if password.is_empty() {
            let _ = CredentialManager::delete(&id);
        } else {
            CredentialManager::save(&id, &password)?;
            // Verify credential was stored correctly
            let stored = CredentialManager::get(&id)?;
            if stored.as_deref() != Some(password.as_str()) {
                log::warn!("Credential verification failed for connection {}: stored_len={:?}, expected_len={}",
                    id, stored.as_ref().map(|s| s.len()), password.len());
            } else {
                log::info!("Credential updated and verified for connection {} (len={})", id, password.len());
            }
        }
    }

    Ok(conn)
}

#[command]
pub async fn delete_connection(
    app: tauri::AppHandle,
    id: String,
) -> Result<bool, AppError> {
    let storage = app.state::<StorageState>().inner().clone();
    storage.delete_connection(&id).await?;
    let _ = CredentialManager::delete(&id);
    Ok(true)
}

#[command]
pub async fn list_connections(
    app: tauri::AppHandle,
) -> Result<Vec<Connection>, AppError> {
    let storage = app.state::<StorageState>().inner().clone();
    storage.list_connections().await
}

#[command]
pub async fn get_connection_by_id(
    app: tauri::AppHandle,
    id: String,
) -> Result<Connection, AppError> {
    let storage = app.state::<StorageState>().inner().clone();
    storage.get_connection(&id).await
}

#[command]
pub async fn reorder_connections(
    app: tauri::AppHandle,
    ids: Vec<String>,
) -> Result<bool, AppError> {
    let storage = app.state::<StorageState>().inner().clone();
    storage.reorder_connections(&ids).await?;
    Ok(true)
}

#[command]
pub async fn test_connection(
    app: tauri::AppHandle,
    id: String,
) -> Result<TestResult, AppError> {
    let storage = app.state::<StorageState>().inner().clone();
    let conn = storage.get_connection(&id).await?;

    let password = CredentialManager::get(&id)?
        .unwrap_or_default();

    let config: serde_json::Value =
        serde_json::from_str(&conn.config_json).unwrap_or_default();
    let driver = config.get("driver").and_then(|v| v.as_str()).unwrap_or("mysql");
    let database = config.get("database").and_then(|v| v.as_str()).unwrap_or("");
    let db_opt = if database.is_empty() { None } else { Some(database.to_string()) };

    // 从配置中解析 SSL 配置
    let ssl_config: Option<SslConfig> = config.get("ssl")
        .and_then(|v| serde_json::from_value(v.clone()).ok());

    // 从配置中解析连接池配置
    let pool_config: Option<PoolConfig> = config.get("pool")
        .and_then(|v| serde_json::from_value(v.clone()).ok());

    match DbEngine::test_connect(
        driver.to_string(),
        conn.host.clone(),
        conn.port,
        conn.username.clone(),
        password.clone(),
        db_opt,
        ssl_config,
        pool_config,
    ).await {
        Ok(latency) => Ok(TestResult {
            success: true,
            message: format!("Connected to {}:{} ({}ms)", conn.host, conn.port, latency),
            latency_ms: Some(latency),
        }),
        Err(e) => Ok(TestResult {
            success: false,
            message: e.to_string(),
            latency_ms: None,
        }),
    }
}

#[command]
pub async fn test_connection_params(
    host: String,
    port: u16,
    username: String,
    password: String,
    database: Option<String>,
    driver: Option<String>,
    ssl_config: Option<SslConfig>,
    pool_config: Option<PoolConfig>,
) -> Result<TestResult, AppError> {
    let drv = driver.as_deref().unwrap_or("mysql");
    let db_opt = database.filter(|d| !d.is_empty());

    match DbEngine::test_connect(
        drv.to_string(),
        host.clone(),
        port,
        username.clone(),
        password.clone(),
        db_opt,
        ssl_config,
        pool_config,
    ).await {
        Ok(latency) => Ok(TestResult {
            success: true,
            message: format!("Connected to {}:{} ({}ms)", host, port, latency),
            latency_ms: Some(latency),
        }),
        Err(e) => Ok(TestResult {
            success: false,
            message: e.to_string(),
            latency_ms: None,
        }),
    }
}

// --- Groups ---

#[command]
pub async fn list_groups(
    app: tauri::AppHandle,
) -> Result<Vec<ConnectionGroup>, AppError> {
    let storage = app.state::<StorageState>().inner().clone();
    storage.list_groups().await
}

#[command]
pub async fn create_group(
    app: tauri::AppHandle,
    name: String,
) -> Result<ConnectionGroup, AppError> {
    let storage = app.state::<StorageState>().inner().clone();
    let group = ConnectionGroup {
        id: Uuid::new_v4().to_string(),
        name,
        sort_order: 0,
        parent_id: None,
    };

    storage.create_group(&group).await?;
    Ok(group)
}

#[command]
pub async fn delete_group(
    app: tauri::AppHandle,
    id: String,
) -> Result<bool, AppError> {
    let storage = app.state::<StorageState>().inner().clone();
    storage.delete_group(&id).await?;
    Ok(true)
}

/// 更新分组：支持重命名和移动分组到新的父级
/// - group_id: 要更新的分组 ID
/// - name: 新的分组名称
/// - parent_id: 新的父级分组 ID（None 表示移到根级，Some("") 也视为根级）
/// 移动分组时会校验：不能移动到自身或其后代下，且嵌套深度不能超过 3 级
#[command]
pub async fn update_group(
    app: tauri::AppHandle,
    group_id: String,
    name: String,
    parent_id: Option<String>,
) -> Result<ConnectionGroup, AppError> {
    let storage = app.state::<StorageState>().inner().clone();
    // 获取当前分组
    let mut group = storage.get_group(&group_id).await?;

    // 规范化 parent_id：空字符串视为 None（根级）
    let new_parent_id = parent_id.filter(|p| !p.is_empty());

    // 如果要移动到新的父级分组，需要进行校验
    if new_parent_id != group.parent_id {
        if let Some(ref target_parent_id) = new_parent_id {
            // 不能移动到自身
            if target_parent_id == &group_id {
                return Err(AppError::Validation("不能将分组移动到自身下".into()));
            }

            // 不能移动到自己的后代分组下（防止循环引用）
            let is_descendant = storage
                .is_descendant_of(&group_id, target_parent_id)
                .await?;
            if is_descendant {
                return Err(AppError::Validation("不能将分组移动到其子分组下".into()));
            }

            // 校验嵌套深度：目标父级深度 + 当前分组子树深度 + 1（当前分组自身）不能超过 3
            let parent_depth = storage
                .get_group_depth(target_parent_id)
                .await?;
            let children_depth = storage
                .get_max_children_depth(&group_id)
                .await?;
            let total_depth = parent_depth + 1 + children_depth;

            if total_depth > 3 {
                return Err(AppError::Validation(format!(
                    "分组嵌套深度不能超过 3 级（当前操作将导致 {} 级嵌套）",
                    total_depth
                )));
            }
        }
    }

    // 更新分组信息
    group.name = name;
    group.parent_id = new_parent_id;

    storage.update_group(&group).await?;
    Ok(group)
}

/// 移动连接到指定分组
/// - connection_id: 要移动的连接 ID
/// - target_group_id: 目标分组 ID（None 或空字符串表示移到根级）
#[command]
pub async fn move_connection(
    app: tauri::AppHandle,
    connection_id: String,
    target_group_id: Option<String>,
) -> Result<bool, AppError> {
    let storage = app.state::<StorageState>().inner().clone();
    // 规范化：空字符串视为 None（根级）
    let group_id = target_group_id.filter(|g| !g.is_empty());

    // 如果指定了目标分组，验证分组是否存在
    if let Some(ref gid) = group_id {
        storage.get_group(gid).await?;
    }

    storage
        .move_connection_to_group(&connection_id, group_id.as_deref())
        .await?;

    Ok(true)
}

/// 切换连接的收藏状态
/// 在连接的 config_json 中切换 isFavorite 字段
/// - connection_id: 要切换收藏状态的连接 ID
/// 返回切换后的收藏状态（true=已收藏，false=未收藏）
#[command]
pub async fn toggle_favorite(
    app: tauri::AppHandle,
    connection_id: String,
) -> Result<bool, AppError> {
    let storage = app.state::<StorageState>().inner().clone();
    // 获取当前连接
    let mut conn = storage
        .get_connection(&connection_id)
        .await?;

    // 解析 config_json
    let mut config: serde_json::Value =
        serde_json::from_str(&conn.config_json).unwrap_or_default();

    // 切换 isFavorite 字段
    let current_favorite = config
        .get("isFavorite")
        .and_then(|v| v.as_bool())
        .unwrap_or(false);
    let new_favorite = !current_favorite;

    config["isFavorite"] = serde_json::Value::Bool(new_favorite);

    // 更新 config_json 和时间戳（serde_json::Error 已有 From impl，直接用 ?）
    conn.config_json = serde_json::to_string(&config)?;
    conn.updated_at = Utc::now().timestamp_millis();

    storage
        .update_connection(&conn)
        .await?;

    Ok(new_favorite)
}

// --- Credential ---

#[command]
pub async fn get_credential(id: String) -> Result<Option<String>, AppError> {
    CredentialManager::get(&id)
}

#[command]
pub async fn save_credential(id: String, password: String) -> Result<bool, AppError> {
    CredentialManager::save(&id, &password)?;
    Ok(true)
}

#[command]
pub async fn delete_credential(id: String) -> Result<bool, AppError> {
    CredentialManager::delete(&id)?;
    Ok(true)
}

// --- App ---

#[command]
pub async fn get_app_version() -> String {
    env!("CARGO_PKG_VERSION").to_string()
}

// --- Types ---

#[derive(Debug, serde::Serialize)]
#[serde(rename_all = "camelCase")]
pub struct TestResult {
    pub success: bool,
    pub message: String,
    pub latency_ms: Option<u64>,
}

#[command]
pub async fn update_boot_config(
    app: tauri::AppHandle,
    data_storage_path: Option<String>
) -> Result<(), AppError> {
    let storage = app.state::<StorageState>().inner().clone();
    // 1. 获取当前的数据库路径（老家）
    let current_db_path = storage.get_db_path_raw();

    // 2. 如果有新路径，准备执行"搬迁"
    if let Some(ref new_dir_str) = data_storage_path {
        let new_dir = std::path::PathBuf::from(new_dir_str);
        let new_db_path = new_dir.join("devforge.db");

        // 确保新目录存在（std::io::Error 已有 From impl，直接用 ?）
        if !new_dir.exists() {
            std::fs::create_dir_all(&new_dir)?;
        }

        // 物理迁移文件：如果老家有东西，且新家没东西，就搬过去
        if current_db_path.exists() && !new_db_path.exists() {
            log::info!("正在从 {:?} 物理搬迁数据到 {:?}", current_db_path, new_db_path);
            // 先尝试复制（std::io::Error 已有 From impl，直接用 ?）
            std::fs::copy(&current_db_path, &new_db_path)?;

            // 复制成功后，尝试把老家的改个名（如果被锁定则跳过，不影响新家）
            let backup_path = current_db_path.with_extension("db.bak");
            if let Err(e) = std::fs::rename(&current_db_path, backup_path) {
                log::warn!("老家数据库文件被锁定，暂无法自动重命名(不影响搬迁): {}", e);
            }
        }
    }

    // 3. 保存引导配置，指路"新家"（BootConfigManager::save 返回 Result<(), String>，需要转换）
    let config = crate::utils::boot_config::BootConfig {
        data_storage_path,
    };
    crate::utils::boot_config::BootConfigManager::save(&config)
        .map_err(|e| AppError::Other(e))?;

    // 4. 即时重载存储引擎，让"搬迁"立即生效
    storage.reload().await?;

    Ok(())
}

#[command]
pub async fn reload_storage(
    app: tauri::AppHandle,
) -> Result<(), AppError> {
    let storage = app.state::<StorageState>().inner().clone();
    storage.reload().await
}

#[command]
pub fn show_main_window(window: tauri::Window) {
    let _ = window.show();
}

#[command]
pub fn get_suggested_data_path() -> String {
    crate::utils::boot_config::BootConfigManager::get_suggested_default_path()
        .to_string_lossy()
        .to_string()
}
