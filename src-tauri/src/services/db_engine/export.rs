...
impl DbEngine {
    pub async fn export_data(
        ...
        let export_table_name = request.options.sql_table_name.as_deref().unwrap_or(&table_name);
        ...
        if total_rows <= BATCH_THRESHOLD {
            ...
            self.write_export_file(&request.file_path, &request.format, &request.options, &result.columns, &result.rows, export_table_name, mysql_pool, &request.source).await
        } else {
            self.export_data_batched(mysql_pool, &query_sql, total_rows, BATCH_SIZE, &request.file_path, &request.format, &request.options, export_table_name, app, &request.source).await
        }
    }

    async fn export_data_batched(&self, pool: &sqlx::MySqlPool, base_sql: &str, total_rows: u64, batch_size: u64, file_path: &str, format: &ExportFormat, options: &ExportOptions, table_name: &str, app: &tauri::AppHandle, source: &ExportSource) -> Result<ExportResult, AppError> {
        let file = std::fs::File::create(file_path).map_err(|e| AppError::Other(e.to_string()))?;
        let mut writer = std::io::BufWriter::new(file);

        if matches!(format, ExportFormat::Excel) {
            return self.export_excel_batched(pool, base_sql, total_rows, batch_size, file_path, app).await;
        }
        ...
        for batch_idx in 0..total_batches {
            ...
            self.write_export_rows(&mut writer, format, options, &columns_cache, &res.rows, table_name, total_exported > 0)?;
    ...
    fn write_export_header<W: Write>(&self, writer: &mut W, format: &ExportFormat, options: &ExportOptions, columns: &[crate::models::query::ColumnDef]) -> Result<(), AppError> {
        match format {
    ...
    fn write_export_rows<W: Write>(&self, writer: &mut W, format: &ExportFormat, options: &ExportOptions, columns: &[crate::models::query::ColumnDef], rows: &[Vec<serde_json::Value>], table_name: &str, has_prev: bool) -> Result<(), AppError> {
        match format {
            ExportFormat::Csv => {
                let q = options.csv_quote_char.as_deref().unwrap_or("\"");
            ...
            ExportFormat::Sql => {
                ...
                for chunk in rows.chunks(batch) {
                    let vals = chunk.iter().map(|row| format!("({})", row.iter().map(|v| self.json_value_to_sql_literal(v)).collect::<Vec<_>>().join(", "))).collect::<Vec<_>>().join(",\n");
    ...
    async fn write_export_file(&self, path: &str, format: &ExportFormat, options: &ExportOptions, columns: &[crate::models::query::ColumnDef], rows: &[Vec<serde_json::Value>], table_name: &str, pool: &sqlx::MySqlPool, source: &ExportSource) -> Result<ExportResult, AppError> {
        if matches!(format, ExportFormat::Excel) { self.write_excel_file(path, columns, rows)?; }
        else {
            ...
            self.write_export_rows(&mut writer, format, options, columns, rows, table_name, false)?;
            if matches!(format, ExportFormat::Json) { writer.write_all(b"\n]").map_err(|e| AppError::Other(e.to_string()))?; }
            writer.flush().map_err(|e| AppError::Other(e.to_string()))?;
        }
    ...
    fn write_excel_file(&self, path: &str, columns: &[crate::models::query::ColumnDef], rows: &[Vec<serde_json::Value>]) -> Result<(), AppError> {
        ...
        for (r_idx, row) in rows.iter().enumerate() {
            for (c_idx, val) in row.iter().enumerate() { self.write_excel_cell(sheet, (r_idx + 1) as u32, c_idx as u16, val)?; }
        }
        work.save(path).map_err(|e| AppError::Other(e.to_string()))?;
        Ok(())
    }

    fn write_excel_cell(&self, sheet: &mut rust_xlsxwriter::Worksheet, row: u32, col: u16, val: &serde_json::Value) -> Result<(), AppError> {
        match val {
    ...
    async fn export_excel_batched(&self, pool: &sqlx::MySqlPool, base: &str, total: u64, batch: u64, path: &str, app: &tauri::AppHandle) -> Result<ExportResult, AppError> {
        use rust_xlsxwriter::{Workbook, Format};
        let mut work = Workbook::new();
        let sheet = work.add_worksheet();
        let fmt = Format::new().set_bold();
        ...
        for i in 0..counts {
            ...
            for (r_idx, row) in res.rows.iter().enumerate() {
                for (c_idx, v) in row.iter().enumerate() { self.write_excel_cell(sheet, (total_exp as u32) + (r_idx as u32) + 1, c_idx as u16, v)?; }
            }
