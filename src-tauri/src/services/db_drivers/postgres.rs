...
pub fn build_table_data_seek_sql(schema: &str, table: &str, page_size: u32, where_clause: Option<&str>, seek_column: &str, seek_value: i64) -> Result<String, String> {
    let mut sql = format!("SELECT * FROM \"{}\".\"{}\"", escape_pg_ident(schema), escape_pg_ident(table));
    let seek_predicate = format!("\"{}\" > {}", escape_pg_ident(seek_column), seek_value);
    if let Some(w) = where_clause {
        let w = w.trim();
        if !w.is_empty() {
            validate_sql_clause(w)?;
    ...
    sql.push_str(&format!(" ORDER BY \"{}\" ASC LIMIT {}", escape_pg_ident(seek_column), page_size));
    Ok(sql)
}
